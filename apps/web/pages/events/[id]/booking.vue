  ...
  <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
    ...
    <template v-else>
      <div class="flex items-center justify-between mb-6">
        <div>
          <h1 class="text-2xl font-bold text-gray-900">Booking Tiket</h1>
          <p class="text-gray-500">{{ event.title }} — {{ event.venue }}</p>
        </div>
        <NuxtLink v-if="hasActiveSession" :to="`/events/${event.id}`" class="text-sm text-primary-600 hover:text-primary-700">
          Detail Event &rarr;
        </NuxtLink>
      </div>

      <div v-if="hasActiveSession" class="card p-4 mb-6 flex items-center justify-between">
        <div class="flex items-center gap-2 text-sm text-gray-600">
          <svg class="w-5 h-5 text-yellow-500" fill="currentColor" viewBox="0 0 20 20">
            <path fill-rule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm1-12a1 1 0 10-2 0v4a1 1 0 00.293.707l2.828 2.829a1 1 0 101.415-1.415L11 9.586V6z" clip-rule="evenodd" />
          </svg>
          Sesi booking akan berakhir dalam <strong class="text-gray-900 tabular-nums">
            ...
            <div v-for="cat in event.categories" :key="cat.id" class="card p-4">
              <div class="flex items-center justify-between">
                ...
                <div class="flex items-center gap-3">
                  <button
                    class="w-8 h-8 rounded-full border border-gray-300 flex items-center justify-center text-gray-600 hover:bg-gray-50 disabled:opacity-30"
                    :disabled="(quantities[cat.id] || 0) === 0"
                    @click="decrement(cat.id)"
                  >
                    &minus;
                  </button>
                  <span class="w-8 text-center font-semibold tabular-nums">{{ quantities[cat.id] || 0 }}</span>
                  <button
                    class="w-8 h-8 rounded-full border border-gray-300 flex items-center justify-center text-gray-600 hover:bg-gray-50 disabled:opacity-30"
